...
export const getScheduleRecomendationCountByYear = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      const count = await Schedule_Recommendation.count({
        where: {
          // ClearingDate: null,
          MPSDueDate: {
            gte: monthStartDate,
    ...
    console.error("Error menghitung jumlah data Operation :", err);
    res.status(500).json({
      success: false,
      message: "Error menghitung jumlah data Operation",
    });
  }
};
